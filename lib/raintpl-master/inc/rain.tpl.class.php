	...
	protected function check_template( $tpl_name ){
		...
		if( !isset($this->tpl['checked']) ){
			...
			// if the template doesn't exist and is not an external source throw an error
                        $dirs = explode(";", $this->tpl['template_directory']);
                        //var_dump($dirs);
                        foreach ($dirs as $dir) {                            
                            $this->tpl['tpl_filename']          = self::$root_dir . $dir . $tpl_basename . '.' . self::$tpl_ext;    // template filename
			...
			elseif( preg_match( '/\{include="([^"]*)"(?: cache="([^"]*)"){0,1}\}/', $html, $code ) ){
				if (preg_match("/http/", $code[1])) {
					...
					if( isset($code[ 2 ]) ){
                                                ...
                                                //include
                                                $compiled_code .= '<?php $tpl = new '.custom_get_called_class().';' .
