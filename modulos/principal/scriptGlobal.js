...
opcionesDatatable = {
    ...
    sLengthMenu: "<label class='labelSelectDatatable'>Registros</label>" + '<select>' +
...
$(document).ready(function () {
    $(this).scrollTop(0);
    // $("div#mainContainer").overlayScrollbars({
    //     overflowBehavior: {
    //         y: "hidden",
    //     },
    //     scrollbars: {
...
function renderizarSelects() {
    ...
    $("select.form-control.selectpicker").each(function (index, el) {
        ...
        if (value === "" || value === "0" || value.length === 0) {
            $("button.dropdown-toggle[data-id='" + idSelect + "']").css('color', '#6F6F6E');
        } else {
            $("button.dropdown-toggle[data-id='" + idSelect + "']").css('color', '#000000');
        }

        $("button.dropdown-toggle[data-id='" + idSelect + "']").siblings("div.dropdown-menu").find("div.bs-actionsbox button.actions-btn").addClass("btnDefault");

        $("button.dropdown-toggle[data-id='" + idSelect + "']").siblings("div.dropdown-menu").find("div.bs-actionsbox button.bs-select-all").html("Marcar todo");
        $("button.dropdown-toggle[data-id='" + idSelect + "']").siblings("div.dropdown-menu").find("div.bs-actionsbox button.bs-deselect-all").html("Desmarcar todo");
    });
}
...
function configuracionInputError() {
    ...
    $("button.dropdown-toggle").on('focus', function (event) {
        ...
        $(this).prev('select').parent().next('span.spanHelperErrorInput').css('display', 'none');
    });
